...
class MADNet(Stereo_net.StereoNet):
    ...
    def _preprocess_inputs(self, args):
        ...
        #"""
        self._left_input_batch = tf.cast(self._left_input_batch, dtype=tf.float32) / 255.0
        self._left_input_batch = self._left_input_batch - (100.0 / 255)
        #"""
        self._left_input_batch = preprocessing.pad_image(self._left_input_batch, 64, dynamic=True)

        self._right_input_batch = args['right_img']
        #"""
        self._right_input_batch = tf.cast(self._right_input_batch, dtype=tf.float32) / 255.0
        self._right_input_batch = self._right_input_batch - (100.0 / 255)
        #"""
        self._right_input_batch = preprocessing.pad_image(self._right_input_batch, 64, dynamic=True)
    ...
    def _make_disp(self,op,scale):
        if not rescale_flag:
            ...
            op = preprocessing.rescale_image(tf.nn.relu(op * -20), [self._left_input_batch.get_shape()[1].value, self._left_input_batch.get_shape()[2].value])
    ...
    def _stereo_estimator(self, costs, upsampled_disp=None, scope='fgc-volume-filtering'):
        ...
        with tf.variable_scope(scope):
            ...
            ).as_list()[3], 128], name='disp-1', bName='biases', activation=activation,variable_collection=self._variable_collection))
            ...
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 128, 128], name='disp-2', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # disp-3
            names.append('{}/disp3'.format(scope))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 128, 96], name='disp-3', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # disp-4
            names.append('{}/disp4'.format(scope))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 96, 64], name='disp-4', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # disp-5
            names.append('{}/disp5'.format(scope))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 64, 32], name='disp-5', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # disp-6
            names.append('{}/disp6'.format(scope))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 32, 1], name='disp-6', bName='biases', activation=lambda x: x,variable_collection=self._variable_collection))
    ...
    def _stereo_context_net(self, input, disp):
        ...
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(input_layer, [
            3, 3, input_layer.get_shape().as_list()[-1], 128], name='context-1', rate=1, activation=att,variable_collection=self._variable_collection))

        # context-2
        names.append('context2')
        input_layer = self._get_layer_as_input(names[-2])
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(
            input_layer, [3, 3, 128, 128], name='context-2', rate=2, activation=att,variable_collection=self._variable_collection))

        # context-3
        names.append('context3')
        input_layer = self._get_layer_as_input(names[-2])
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(
            input_layer, [3, 3, 128, 128], name='context-3', rate=4, activation=att,variable_collection=self._variable_collection))

        # context-4
        names.append('context4')
        input_layer = self._get_layer_as_input(names[-2])
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(
            input_layer, [3, 3, 128, 96], name='context-4', rate=8, activation=att,variable_collection=self._variable_collection))

        # context-5
        names.append('context5')
        input_layer = self._get_layer_as_input(names[-2])
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(
            input_layer, [3, 3, 96, 64], name='context-5', rate=16, activation=att,variable_collection=self._variable_collection))

        # context-6
        names.append('context6')
        input_layer = self._get_layer_as_input(names[-2])
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(
            input_layer, [3, 3, 64, 32], name='context-6', rate=1, activation=att,variable_collection=self._variable_collection))

        # context-7
        names.append('context7')
        input_layer = self._get_layer_as_input(names[-2])
        self._add_to_layers(names[-1], sharedLayers.dilated_conv2d(
            input_layer, [3, 3, 32, 1], name='context-7', rate=1, activation=lambda x: x,variable_collection=self._variable_collection))
    ...
    def _pyramid_features(self, input_batch, scope='pyramid', reuse=False, layer_prefix='pyramid'):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            )[-1].value, 16], strides=2, name='conv1', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv2
            names.append('{}/conv2'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 16, 16], strides=1, name='conv2', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv3
            names.append('{}/conv3'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 16, 32], strides=2, name='conv3', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv4
            names.append('{}/conv4'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 32, 32], strides=1, name='conv4', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv5
            names.append('{}/conv5'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 32, 64], strides=2, name='conv5', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv6
            names.append('{}/conv6'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 64, 64], strides=1, name='conv6', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv7
            names.append('{}/conv7'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 64, 96], strides=2, name='conv7', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv8
            names.append('{}/conv8'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 96, 96], strides=1, name='conv8', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv9
            names.append('{}/conv9'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 96, 128], strides=2, name='conv9', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv10
            names.append('{}/conv10'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 128, 128], strides=1, name='conv10', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv11
            names.append('{}/conv11'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 128, 192], strides=2, name='conv11', bName='biases', activation=activation,variable_collection=self._variable_collection))

            # conv12
            names.append('{}/conv12'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [
                3, 3, 192, 192], strides=1, name='conv12', bName='biases', activation=activation,variable_collection=self._variable_collection))
    ...
    def _build_network(self, args):
        ...
        with tf.variable_scope("G6"):
            ...
            if not rescale_flag:
                ...
                u5 = preprocessing.rescale_image(V6, [image_height // scales[5], image_width // scales[5]]) * 20. / scales[5]
            if args['bulkhead']:
        ...
        with tf.variable_scope("G5"):
            ...
            if not rescale_flag:
                ...
                u4 = preprocessing.rescale_image(V5, [image_height // scales[4], image_width // scales[4]]) * 20. / scales[4]
            if args['bulkhead']:
        ...
        with tf.variable_scope('G4'):
            ...
            if not rescale_flag:
                ...
                u3 = preprocessing.rescale_image(V4, [image_height // scales[3], image_width // scales[3]]) * 20. / scales[3]
            if args['bulkhead']:
        ...
        with tf.variable_scope('G3'):
            ...
            if not rescale_flag:
                ...
                u2 = preprocessing.rescale_image(V3, [image_height // scales[2], image_width // scales[2]]) * 20. / scales[2]
            if args['bulkhead']:
        ...
        if not rescale_flag:
            rescaled_prediction = tf.nn.relu(
                tf.image.resize_images(self._get_layer_as_input('final_disp'), [image_height, image_width]) * -20.)
        else:
            rescaled_prediction = tf.nn.relu(
                preprocessing.rescale_image(self._get_layer_as_input('final_disp'), [image_height, image_width]) * -20.)
